...
<head>
  <?PHP 
    ...
    if(array_key_exists('hidIsReturning',$_POST)) {
      ...
      if(isset($_POST["lstDisplay"])) {
        ...
        switch($selection) {
          ...
          case "band": {
            $tableFormat = BAND;
            $sql = "SELECT * FROM band ORDER BY bandName";
            break;
          }
          case "songAlbum": {
          ...
          default: echo($selection . "is not a valid choice from the list of displays<br />");
          
        } // end of switch()
      } // end of if()
    }
    // or, a first time visit?
    else {
    ...
    function displayData() {
      ...
      $db = new mysqli("localhost", "root", "mysql", "classicVinyl");

      if($db->connect_errno > 0) {
        die("Unable to connect to database [" . $db->connect_error. "]");
      }

      // Get the data from the databse using SQL
...
<body>
  <div id="frame">
    ...
    <form name="frmDBF" action="<?PHP echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
      ...
      <noscript>
        &nbsp; &nbsp; &nbsp;
        <input type="submit" name="btnSubmit" value="View in list" />
        <br /><br />
      </noscript>

      <!-- Use a hidden field to tell serer if return visitor -->
