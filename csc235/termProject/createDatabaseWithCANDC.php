...
<body>
  ...
  ************************************************/
  ...
  $recipeArray = array(
    array(3, "Enchilada Hotdish", "Yummy hotdish!", "1 peice", 18),
    array(3, "White Chicken Enchilada", "Yummy enchiladas!", "2 enchiladas", 10),
    array(2, "Penna Rosa", "Lovely pasta", "1 bowl", 6),
    array(1, "Crispy Waffles", "My favorite recipe from my mother!", "1 waffle", 4)
  );

  foreach($recipeArray as $recipe) {
    $sql = "INSERT INTO recipe (collectionID, recipeName, recipeDescription, servingSize, numServings) "
      . "VALUES ('" . $recipe[0] . "', '"
      . $recipe[1] . "', '"
      . $recipe[2] . "', '"
      . $recipe[3] . "', '"
      . $recipe[4] . "')";
    runQuery($sql, "Record inserted for: " . $recipe[1], true);
  }
  ...
  $ingredientArray = array(
    array(1, "Milk", "Delicious liquid!"),
    array(1, "Eggs", "Straight from the bird!"),
    array(3, "Garlic Powder", "A very versatile spice that uses garlic"),
    array(2, "Jerkey Sticks", "Beef jerky stick")
  );

  foreach($ingredientArray as $ingredient) {
    $sql = "INSERT INTO ingredient (categoryID, ingredientName, ingredientDescription) "
      . "VALUES ('" . $ingredient[0] . "', '"
      . $ingredient[1] . "', '"
      . $ingredient[2] . "')";
    runQuery($sql, "Record inserted for: " . $ingredient[1], true);
  }
  ...
  foreach($ingredientArray as $ingredient) {
    buildIngredientRecipe($ingredient[1], $recipeArray[0][1], "1");
    buildIngredientRecipe($ingredient[1], $recipeArray[1][1], "1");
    buildIngredientRecipe($ingredient[1], $recipeArray[2][1], "1");
    buildIngredientRecipe($ingredient[1], $recipeArray[3][1], "1");
  }
